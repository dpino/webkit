...
// setTimeout, because async rejections get swallowed in jsc.cpp.
...
function afterCycles()
{
    ...
    // Back-to-back changes must each take effect at the next VM entry.
    if (!$vm.setHostTimeZone("America/New_York"))
        throw new Error("Failed to set host time zone to America/New_York");
    setTimeout(() => {
        expect("host-TZ change cacheable path",
            new Intl.DateTimeFormat("en-US").resolvedOptions().timeZone, "America/New_York");

        if (!$vm.setHostTimeZone("Australia/Sydney"))
            throw new Error("Failed to set host time zone to Australia/Sydney");
        setTimeout(() => {
